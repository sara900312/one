...
import { formatError, handleError } from "@/utils/errorHelpers";
...
const OrdersList: React.FC<OrdersListProps> = ({ refreshTrigger }) => {
    ...
    try {
      ...
      if (user?.role === "store" && user.store_id) {
        filteredOrders = filteredOrders.filter(
          (order) => order.assigned_store === user.store_id,
        );
      }
  ...
  const fetchStores = async () => {
    try {
      ...
      await handleError("جلب المتاجر", error, {
        context: { operation: "fetch_stores_general_error" },
        fallbackMessage: "حدث خطأ في تحميل المتاجر",
      });
    }
  };
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-semibold text-ar">
                          {order.customer_name}
                        </span>
                        {order.customer_code && (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-ar">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            {order.product_name} (الكمية: {order.quantity})
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="font-medium">
                            المبلغ الإجمالي:{" "}
                            {order.total_amount.toLocaleString("ar-SA")} ريال
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Select
                          onValueChange={(storeId) =>
                            handleAssignOrder(order.id, storeId)
                          }
