  ...
  const handlePublishProduct = async (productId: string) => {
    ...
    try {
      ...
      if (response?.success) {
        ...
        toast.error(response?.error || 'فشل في نشر المنتج')
      }
    } catch (error) {
      console.error('Publish Product Error:', error)
      toast.error('حدث خطأ في نشر المنتج')
    }
  }
                ...
                <CardContent className="p-4">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1">
                      <h3 className="font-semibold text-ar text-lg">{product.name}</h3>
                      <p className="text-sm text-muted-foreground text-ar">{product.store_name}</p>
                      {product.description && (
                        <p className="text-sm text-ar mt-2">{product.description}</p>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    ...
                    {product.discount_amount > 0 && (
                      <div className="text-ar">
                        ...
                        <span>{product.discount_amount.toLocaleString('ar-SA')} ريال</span>
                      </div>
                    )}
                    {product.final_price && (
                      <div className="text-ar">
                        <span className="font-medium">السعر النهائي: </span>
                        <span>{product.final_price.toLocaleString('ar-SA')} ريال</span>
                      </div>
                    )}
                  </div>
                  
                  <div className="flex gap-2 flex-wrap">
                    {product.can_edit && product.status === 'draft' && user?.role === 'admin' && (
                      <Button
                        size="sm"
