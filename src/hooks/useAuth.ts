  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      logError(
        ...
        { email, hasUserError: !!userError, hasStoreError: !!storeError },
      );

      return { success: false, error: "بيانات تسجيل الدخول غير صحيحة" };
    } catch (error) {
      const formattedError = formatError(error);
      logError("تسجيل الدخول - خطأ عام", error, { email });
      return {
        success: false,
        error: formattedError.message || "حدث خطأ أثناء تسجيل الدخول",
      };
    } finally {
      setLoading(false);
    }
